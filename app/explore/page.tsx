...
export default function ExplorePage() {
  ...
  const filters = [
    ...
    { id: 'active', label: 'Recently Active', count: users.filter(u => u.last_active.includes('hour')).length },
    { id: 'streaks', label: 'High Streaks', count: users.filter(u => u.mood_streak && u.mood_streak > 10).length },
    { id: 'popular', label: 'Popular', count: users.filter(u => u.followers_count > 300).length }
  ];
  ...
  useEffect(() => {
    ...
    switch (selectedFilter) {
      case 'active':
        ...
        break;
      case 'streaks':
        filtered = users.filter(user => user.mood_streak && user.mood_streak > 10);
        break;
      case 'popular':
    ...
    if (searchQuery) {
      filtered = filtered.filter(user =>
        ...
        user.wellness_focus.some(focus => focus.toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }
      ...
      <div className="min-h-screen bg-linear-to-br from-purple-50 via-pink-50 to-blue-50">
        ...
        <div className="flex justify-center items-center h-64">
          <div className="text-center bg-white rounded-xl p-8 shadow-sm">
            ...
            <h2 className="text-xl font-semibold text-gray-800 mb-2">Explore Our Community</h2>
            <p className="text-gray-600 mb-6">Sign in to discover and connect with amazing people</p>
            <button 
    ...
    <div className="min-h-screen bg-linear-to-br from-purple-50 via-pink-50 to-blue-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-purple-800 mb-4">🔍 Explore</h1>
          <p className="text-xl text-gray-600 mb-6 max-w-3xl mx-auto">
            Discover amazing people in our wellness community and build meaningful connections
          </p>
        </div>
            ...
            <div key={user.id} className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 ${getAvatarColor(user.full_name)} rounded-full flex items-center justify-center text-white font-semibold`}>
                ...
                {user.mood_streak && user.mood_streak > 0 && (
                  <div className="text-center">
                    <div className="text-orange-500 font-bold text-lg">{user.mood_streak}</div>
              ...
              {/* Wellness Focus */}
              <div className="mb-4">
                <div className="text-xs text-gray-500 mb-2">Wellness Focus:</div>
                <div className="flex flex-wrap gap-1">
                  {user.wellness_focus.map((focus) => (
                    <span key={focus} className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded">
                      {focus}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex gap-2">
                ...
                >
                  {followingUsers.has(user.id) || user.is_following ? 'Following' : 'Follow'}
                </button>
                <button
        ...
        {filteredUsers.length === 0 && (
          <div className="text-center py-12 bg-white rounded-xl shadow-sm">
            ...
            <p className="text-gray-600 mb-6">
              Try adjusting your search terms or filters to discover more community members.
            </p>
            <button
              onClick={() => {
                setSearchQuery('');

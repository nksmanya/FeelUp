...
export async function POST(req: Request) {
  try {
    const { user_email, title, description, category, target_date } = await req.json();
...
export async function PATCH(req: Request) {
  try {
    const { goal_id, user_email, completed, mood_at_completion, reflection_note } = await req.json();
    
    if (!goal_id || !user_email) {
      return NextResponse.json({ error: 'Goal ID and user email required' }, { status: 400 });
    }

      ...
      filteredMessages = messages.filter(
        (message: any) =>
          (message.sender_email === userEmail &&
            message.recipient_email === otherUserEmail) ||
          (message.sender_email === otherUserEmail &&
  ...
  try {
    ...
    if (!filterNegativeContent(content)) {
      return NextResponse.json(
        {
          error:
            "Message contains inappropriate content. Please keep messages positive and supportive.",
        },
        { status: 400 },
      );
    }

...
export default function ProfilePage() {
  ...
  const loadStreaks = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/streaks?user_email=${encodeURIComponent(user.email)}`);
      const data = await res.json();
      if (res.ok) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex items-start gap-6">
            <div className="w-24 h-24 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
              <span className="text-white text-3xl font-bold">
                {(profile?.full_name || user.name || user.email || 'U')[0].toUpperCase()}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 soft-glow text-center">
            <div className="text-3xl font-bold text-blue-600 mb-2">
              {streaks.find(s => s.streak_type === 'overall')?.current_count || 0}
            </div>
        ...
        <div className="bg-white rounded-xl p-8 soft-glow mb-8">
          ...
          {achievements.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Start completing goals to unlock achievements!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {achievements.map((achievement) => (
                <div key={achievement.id} className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg p-4 border border-yellow-200">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{achievement.badge_emoji}</span>
                    <div>
                      <div className="font-semibold text-gray-900">{achievement.badge_name}</div>
        ...
        <div className="bg-white rounded-xl p-8 soft-glow">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">🔥 Your Streaks</h2>
          
          {streaks.length === 0 ? (
            <div className="text-center py-8">
              <div className="text-4xl mb-4">🎯</div>
              <p className="text-gray-600">Complete daily goals to start building streaks!</p>
            </div>
          ) : (

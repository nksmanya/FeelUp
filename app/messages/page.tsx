...
const mockMessages: Message[] = [
  {
    ...
    content: 'Hi! I saw your post about meditation and wanted to thank you for sharing those techniques.',
  ...
  },
  {
    ...
    content: "You're welcome! I'm so glad they helped. Have you tried the breathing exercises too?",
  ...
  },
  {
    ...
    sender_email: 'sarah.chen@example.com',
    recipient_email: 'user@example.com',
    content: 'Yes! The 4-7-8 technique has been amazing for my anxiety. Thank you for the meditation tips! They really helped.',
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    if (userParam) {
      const conversation = conversations.find(conv => conv.user_id === userParam);
      if (conversation) {
  ...
  const loadMessages = async (userEmail: string) => {
    ...
    try {
      ...
      const conversationMessages = mockMessages.filter(
        msg => 
          (msg.sender_email === userEmail && msg.recipient_email === session?.user?.email) ||
          (msg.sender_email === session?.user?.email && msg.recipient_email === userEmail)
      );
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !selectedConversation || !session?.user?.email) return;
      ...
      <div className="min-h-screen bg-linear-to-br from-purple-50 via-pink-50 to-blue-50">
        ...
        <div className="flex justify-center items-center h-64">
          <div className="text-center bg-white rounded-xl p-8 shadow-sm">
            ...
            <p className="text-gray-600 mb-6">Sign in to connect and chat with community members</p>
            <button 
          ...
          <div className="flex h-full">
            ...
            <div className="w-1/3 border-r border-gray-200 flex flex-col">
              ...
              <div className="p-4 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-500 mt-1">Stay connected with your wellness community</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {conversations.map((conversation) => (
                  <div
                    ...
                    className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors ${
                      selectedConversation === conversation.user_email ? 'bg-purple-50 border-purple-200' : ''
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 ${getAvatarColor(conversation.user_name)} rounded-full flex items-center justify-center text-white font-semibold shrink-0`}>
                        {conversation.user_name.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          <h3 className="font-medium text-gray-900 truncate">{conversation.user_name}</h3>
                ...
                {conversations.length === 0 && (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-4">Start connecting with community members!</p>
                    <button
                ...
                <>
                  ...
                  <div className="p-4 border-b border-gray-200">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 ${getAvatarColor(conversations.find(c => c.user_email === selectedConversation)?.user_name || '')} rounded-full flex items-center justify-center text-white text-sm font-semibold`}>
                        {conversations.find(c => c.user_email === selectedConversation)?.user_name.charAt(0)}
                      </div>
                      <div>
                        <h2 className="font-medium text-gray-900">
                          {conversations.find(c => c.user_email === selectedConversation)?.user_name}
                        </h2>
                        <p className="text-sm text-gray-500">Active in wellness community</p>
                      </div>
                    </div>
                  </div>
                          ...
                          >
                            ...
                            >
                              ...
                              <p className={`text-xs mt-1 ${
                                message.sender_email === session.user?.email ? 'text-purple-200' : 'text-gray-500'
                              }`}>
                ...
                <div className="flex-1 flex items-center justify-center p-8">
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-6">
                      Select a conversation to start chatting, or explore the community to find new connections.
                    </p>
                    <button
                      onClick={() => router.push('/explore')}
                      className="btn-primary px-6 py-3 rounded-lg"
